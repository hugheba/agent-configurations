...
1. Follow the phase structure exactly as specified
1. No verbose explanations unless explicitly requested
1. No announcements about what phase you're executing
1. Phases must be executed one at a time in sequential order
...
1. Populate the file with:
    - **Request**: Brief summary of what the user asked for
    - **Status**: `In Progress`
    - **Started**: Current timestamp
1. Work silently - do not explain what you're doing
...
1. Update `Agent-Processing.md` after each completion:
    - Mark items as `[x]` when done
    - Add brief notes on any issues or deviations
1. If blocked, document the blocker and continue with other items
...
1. Inform the user that processing is complete
1. Remind user they can delete `Agent-Processing.md` when ready
...
1. Performance, Reliability & Observability
...
1. Immutable + pure logic in domain modules; side‑effects at edges.
1. Small, composable modules; explicit boundaries (no cyclic deps).
1. Deterministic builds (locked versions, repeatable env via devcontainers).
1. Automate everything (format, lint, test, security scan) in CI.
1. Security & observability are baseline features, not add‑ons.
1. Prefer standard library over third‑party; justify each dependency.
1. Documentation lives with code; updated in same PR as change.
1. Performance measured, not guessed (add benchmarks before tuning).
1. Shift‑left on quality: reject code that cannot pass native / strict toolchains when required.
...
1. Hosted configuration (JSON or YAML) containing a single structured document:
   {
   "version": 5,
   "flags": {
...
1. Use AppConfig deployment strategies (canary, linear) rather than bespoke rollout code.
1. Enable AppConfig validators (JSON schema) to reject invalid flag sets before deployment.
...
1. **No Secrets in Example Files**: `.env.example` must only contain:
    - Variable names
...
1. Install pre-commit hooks: `pre-commit install` (or equivalent for your tooling)
1. Fill in local secrets in `.env`
1. Verify startup: app should fail clearly if misconfigured
...
1. Unit Tests (≥70% meaningful coverage; exclude generated code)
1. Integration Tests
1. Security (dependencies + SAST + secret scan)
1. Native / Prod Artifact build
1. Smoke / Health startup test
1. SBOM + provenance + artifact signing
1. Performance smoke (optional gate)
...
- **Minimum Iterations**: Execute minimum 100 iterations per property to ensure comprehensive validation
- **Clear Counterexamples**: When property tests fail, provide clear counterexamples and shrunk inputs for debugging
- **Business Logic Validation**: Use property-based tests to validate business logic comprehensively and catch edge cases across input ranges
- **Precision Testing**: For currency calculations, validate precision across all operations to prevent rounding errors
- **Round-Trip Properties**: For data transformations (serialization, encoding, etc.), verify round-trip properties (encode → decode = identity)
- **Input Variation**: For validation logic, generate comprehensive input variations including edge cases, boundary values, and invalid inputs
- **Recommended Libraries**:
  - **Java/Kotlin**: jqwik, QuickTheories, KotlinTest property testing
  - **JavaScript/TypeScript**: fast-check, jsverify
  - **Python**: Hypothesis
  - **C#/.NET**: FsCheck, CsCheck
  - **Rust**: proptest, quickcheck

**Example Property Tests**:

- **Associativity**: `(a + b) + c = a + (b + c)` for all numeric operations
...
- **Invariants**: Data structure invariants maintained after operations
...
1. Compile / type-check (warnings treated as errors).
1. Unit tests (parallel).
1. Integration tests (Testcontainers).
1. Security scans (SAST + dependencies + secrets).
1. Build artifacts (JAR, native, Node dist, containers).
1. Native tests / smoke.
1. SBOM + provenance + signing.
1. Performance smoke (optional threshold gate).
...
1. **Auto-scaling**: Use serverless/auto-scaling for non-prod (scale to zero)
1. **Scheduled shutdown**: Shut down dev/staging resources outside business hours
1. **Ephemeral environments**: Create on-demand, destroy after use (preview environments)
1. **Shared resources**: Use same monitoring, logging, CI/CD infrastructure
1. **Database branching**: Leverage copy-on-write (Neon) vs full database copies
...
1. `terraform plan` - Preview changes (review in PR)
1. `terraform apply` - Apply changes (in CI/CD or with approval)
1. `terraform destroy` - Clean up resources (with caution)
...
1. Semi-structured NoSQL via JSONB (selective schema flexibility)
1. Vector similarity (pgvector) for embeddings / hybrid search
1. Geospatial (PostGIS) for location, distance, routing enrichment
1. Durable lightweight message queue (pgmq) replacing RabbitMQ for moderate throughput
1. Time-series (TimescaleDB for compression + continuous aggs)
1. Consolidated persistence to reduce operational sprawl (avoid polyglot unless justified)
...
1. Passes and characterizes existing behavior for pure refactors (no semantic change intended).

Process (Red / Green / Refactor enforced):

1. Identify target lines (file + function/method). Keep diff minimal.
1. Search for existing tests referencing the symbol(s). If found, confirm they execute the exact lines (use coverage or temporary breakpoint/log/"fail fast" assertion locally—revert any transient instrumentation before commit).
1. If NO direct test exists, write the smallest focused unit/contract test that would catch an incorrect implementation of the intended change. Name it to reflect intent (e.g., DistanceServiceTest.shouldComputeHaversineDistanceAccurately) and, if helpful, add a brief comment listing the primary lines covered.
1. For bug fixes: write the test to assert the correct (desired) behavior first; run to see it fail (prove necessity).
1. For refactors: create characterization tests capturing current externally observable behavior (inputs/outputs, side-effects). Do NOT change implementation until these pass.
1. Only then modify production code to make tests pass. Keep commits logically ordered (optional: separate commit for failing test, then fix; same PR is required either way).
1. If behavior change affects public API / contract:

- Add/adjust contract or integration tests (HTTP schemas, messaging payloads, DB interactions) plus at least one negative test.

1. Run full test suite (or impacted subset + full CI) before requesting review; ensure coverage for new/changed lines ≥ existing project threshold (never lowers global threshold).
1. Reviewers must block if any changed executable line lacks test execution evidence.
1. Emergency hotfix exception (critical prod outage) may merge with follow-up test commit ONLY if explicitly tagged (HOTFIX) and ticketed; follow-up test must land within 24h.
...
1. Build (JVM JAR + native image / Node dist / container).
1. Run full test suite + native smoke.
1. Generate SBOM + signatures.
1. Publish artifacts (registry, container registry).
1. Create GitHub Release with notes (CHANGELOG excerpt).
1. Deploy via GitHub Actions workflow (env promotion dev -> staging -> prod).
1. Post-deploy smoke + health + metric sanity (error rate, latency, startup).
1. Automated rollback trigger if SLO breach threshold.
